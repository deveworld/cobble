...
| Field | Type | Meaning |
| --- | --- | --- |
| `kind` | string | Resource kind such as `function_tag`, `predicate`, `resource_pack_model`, or `resource_pack_lang`. |
| `namespace` | string | Resource namespace. |
| `path` | string | Resource path without namespace or file extension. |

`resource_pack_model` and `resource_pack_lang` are added in 0.8.0 under the
experimental resource-pack flag. See `docs/resource-pack-design.md`.
