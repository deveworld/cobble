...
Minecraft's `$(param_name)` macro syntax when needed. Cobble currently targets
...
def save_status():
    ...
    storage.get(storage.index("events", 0))
    storage.read_score("event_count", "events", 1)
    storage.remove("status.extra")
```

Storage helpers write to `<namespace>:global`. `storage.path()`,
`storage.child()`, and `storage.index()` return storage path strings for use
inside storage helpers; they do not emit commands when used by themselves.

### Selector, Position, Schedule, Bossbar, Team, And Entity Helpers
...
`selector.*` and `position.*` return plain selector or coordinate strings for
use inside other helpers. They do not emit commands when used by themselves.
These helpers are thin wrappers over Minecraft commands. Use raw commands when
a specialized option is not covered yet.
...
datapack.item_tag("reward_items", [
    "minecraft:diamond",
...
- `datapack.function_tag(name, values, replace=False?)`
- `datapack.block_tag(name, values, replace=False?)`
- `datapack.item_tag(name, values, replace=False?)`
- `datapack.entity_type_tag(name, values, replace=False?)`
...
Tag values may be string resource IDs or object entries with `id` and optional
`required`. Object entries with `required: true` are equivalent to strings;
`required: false` entries remain optional Minecraft tag entries.
Pass an optional third literal boolean argument to emit tag `replace`; when
multiple declarations are merged, `True` wins over `False`.
...
Item component helpers can build `minecraft:set_components` item modifiers
without hiding generated JSON:
...
`item_component.*` helpers return JSON component objects for resource
declarations. They do not emit commands when used by themselves.

### Experimental Resource-Pack Assets

Resource-pack output is opt-in as a 0.9.0 beta feature. Enable it with
...
values must be strings. Multiple lang declarations for the same namespace and
locale merge when repeated keys have identical values; conflicting values are
an error.

When resource-pack output is enabled, Cobble also copies static files from the
project `assets/` directory into the generated pack. Use
`assets/<namespace>/...` for textures, sounds, blockstates, and other
resource-pack files. Static asset paths must stay inside the project
`assets/` tree, use lowercase Minecraft-safe path segments, and must not be
symlinks.
...
Cobble currently requires **Minecraft Java Edition 26.1.2** and pack format **101.1**. Older Minecraft versions are intentionally not supported by this release line.
...
- malformed `datapack.*` helper argument shapes, literal resource IDs, and tag
  value IDs, including invalid tag object entries.
