...
pub struct DataPack {
    ...
    pub pack_format: PackFormat,  // Cobble v0.7.3 requires 101.1
    pub stdlib: StdLib,
...
**Note**: `pack_format` uses the `PackFormat` enum. Cobble v0.7.3 targets Minecraft Java Edition 26.1.2 and requires `PackFormat::Decimal(101, 1)`, serialized into `pack.mcmeta` as `min_format` and `max_format` arrays.
